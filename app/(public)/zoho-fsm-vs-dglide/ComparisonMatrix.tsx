...
import { calculateAccordionScrollTarget } from "../dglide-vs-freshdesk/accordion-scroll";
import styles from "../dglide-vs-freshdesk/ComparisonPage.module.css";

function Value({ value, product }: { value: string; product: "dglide" | "upteams" }) {
  const normalized = value.toLowerCase();
  const tone = normalized.includes("partial") || normalized.includes("not listed")
    ? styles.valueLight
    : styles.valueDark;
  ...
  return (
    <span className={`${styles.value} ${tone}`}>
      <span>{value}</span>
      <span className={styles.srOnly}> for {product === "dglide" ? "DGlide" : "Zoho FSM"}</span>
    </span>
  );
}
                      ...
                      <div className={styles.matrixMobileColumnHeader} aria-hidden>
                        ...
                        <span className={styles.matrixMobileBrand}>
                          <Image
                            ...
                            height={49}
                          />
                          <span className={styles.matrixMobileFreshworksText}>Zoho FSM</span>
                        </span>
                      </div>
                      {group.rows.map((row) => (
                        <div className={styles.matrixRow} key={row.feature}>
                          ...
                          <div className={`${styles.matrixValue} ${styles.matrixValueFreshdesk}`} data-label="Zoho FSM">
                            <Value value={row.upteams} product="upteams" />
                          </div>
                        </div>
                      ))}
                    </div>
