...
function Value({ value, product }: { value: string; product: "dglide" | "upteams" }) {
  ...
  return (
    <span className={`${styles.value} ${tone}`}>
      <span>{value}</span>
      <span className={styles.srOnly}> for {product === "dglide" ? "DGlide" : "UpTeams"}</span>
    </span>
  );
}
                      ...
                      <div className={styles.matrixMobileColumnHeader} aria-hidden>
                        ...
                        <span className={styles.matrixMobileBrand}>
                          <Image
                            ...
                            height={49}
                          />
                          <span className={styles.matrixMobileFreshworksText}>UpTeams</span>
                        </span>
                      </div>
                      {group.rows.map((row) => (
                        <div className={styles.matrixRow} key={row.feature}>
                          ...
                          <div className={`${styles.matrixValue} ${styles.matrixValueFreshdesk}`} data-label="UpTeams">
